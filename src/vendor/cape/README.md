...
(MIT License, Copyright (c) 2026 Victor Zakharov, full text in [LICENSE](LICENSE)), commit
`08aa37baeb4133568c7dc40b4e9a97eed3d598b8`.

- `physics/`, `graphics/proceduralTextures.ts` and `utils/` are **unmodified** copies of the
  CPU/WebGL cape solver and its dependencies, including the web worker that runs it off the
  main thread (`CapePhysicsWorker.ts`, `CapeWorkerProtocol.ts`, `WebGlCapeWorkerPool.ts`).
...
- `world/caveProfile.ts`, `world/CaveShellSampler.ts` and `player/Character.ts` are small
  **shims** that replace the cave world with the Last Stand arena floor
  (`src/world/ground.ts`, imported directly so it also works in the worker) and provide the
  `CapeAnchors` type.
