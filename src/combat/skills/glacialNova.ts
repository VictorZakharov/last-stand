...
const skill: InstantSkill = {
  ...
  cast(player, rawDef) {
    ...
    // pale ice is bright: scaled like the other glows, or the burst whites out a third of the screen
    const g = glowScale(ICE);
    shockwave(c, { color: ICE, intensity: 2 * g, from: 0.5, to: def.radius, life: 0.45 });
    shockwave(c, { color: ICE, intensity: 1.2 * g, from: 0.3, to: def.radius * 0.7, life: 0.3 });
    groundFlash(c, { color: ICE, intensity: 1.2 * g, radius: def.radius, life: 0.5 });
    iceSpikes(c, def.radius * 0.95, 42);
    decal(c, { type: 'frost', size: def.radius * 1.05, life: 6, opacity: 0.5 });

    // high up, so the floor under it is lit broadly instead of burnt white
    flash({ color: ICE, intensity: 35, distance: 18, life: 0.5, pos: { x: c.x, y: 4, z: c.z } });
    addShake(0.3);
    sfx.nova();
    for (let i = 0; i < 160; i++) {
      ...
      particles.glow.spawn({
        ...
        life: rand(0.4, 0.9), size: rand(0.12, 0.35), sizeEnd: 0.02, color: col(i % 3 ? ICE : 0xffffff, 1.0), colorEnd: col(0x2060ff, 0.4), drag: 3.5,
      });
    }
