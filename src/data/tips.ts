// Gameplay tips shown on the loading screen.
export const TIPS: string[] = [
  'After every wave, bank your spoils and leave, or press on for a harder wave and better loot.',
  'Dying loses everything you have not banked.',
  'Press Space in the sanctuary to hide the menus and practice on the training dummies.',
  'Drag spells from the Spellbook onto keys. A spell can sit on several keys, and they share its cooldown.',
  'Right-click gear to equip it. Shift-click it, or drag it to the junk bin, to salvage it.',
  'The mouse wheel zooms the camera in and out. Hold the middle button and drag to rotate it.',
  'Graphics quality adapts on its own. Pick a fixed level in the pause menu (Esc).',
  'After a wave is cleared, B banks your spoils and C continues.',
  'A run can start at any wave you have already beaten in that battleground. The lobby starts you at your best one.',
  'Pick the Crypt or the Forest in the sanctuary, or roll the dice for a random battleground every run.',
];
