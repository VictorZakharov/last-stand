...
- **Bright effects:** light adds up, so the final grade (`core/renderer.ts`) compresses it the way the eye does, and effects and their lights stay at full strength: don't dim single effects to fix a bright scene.
  ...
  - **Distance:** effect materials dim with their distance from the camera (`nearGlow` in `core/materials.ts`: full strength from the top-down camera's distance, down to a quarter up close). `additive()`, the glow particles, projectile cores (a hostile bolt's is an orb of added light, with a pooled light when one is free), entity glows (`kit.glow`, the staff orb included; a hero's gear glows opt out) and the hit flash on bodies have it; wrap a new effect material in `nearGlow()`. The world's own glows don't.
  - **First person:** particles fade within 3.5m of the camera, hostile bolt cores hide within 2.5m, and a cast leaves from `Player.castPoint` / `palmPoint`, which in first person is a hand's place in view (the hidden body's hands are at the camera). Keep lights off the camera: above the head, not at 2m.
