...
| `src/loot/` | Item generation (base names and excluded stats per class), persistent profiles, one per class (`localStorage` `last-stand.profile.<classId>.v1`; the pre-class `last-stand.profile.v1` migrates to the mage), the lobby's class (cookie `last-stand-class`), skill loadout (cookie `last-stand-loadout-<classId>`) |
