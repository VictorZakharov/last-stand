...
- **Bright effects:** light adds up, so the final grade (`core/renderer.ts`) compresses it the way the eye does, and effects and their lights stay at full strength: don't dim single effects to fix a bright scene.
  - **Adaptation:** the average light (after bloom) over a 16x9 and a 4x3 grid of the frame is held under `ADAPT_SMALL` / `ADAPT_LARGE` (`AreaPass`), so a wash over much of the view is scaled down while ordinary scenes, which sit just under the targets, are untouched. Above `HI_KNEE` the brightest channel eases towards `HI_CAP`, and in a large glare tone mapping keeps a bright pixel's colour (`HUE_*`, `GLARE_*`), so a big bright area stays green or blue instead of turning white.
  - **Distance:** effect materials dim with their distance from the camera (`nearGlow` in `core/materials.ts`: full strength from the top-down camera's distance, down to a quarter up close). `additive()` and the glow particles have it; wrap a new effect material in `nearGlow()`. The world's own glows don't.
