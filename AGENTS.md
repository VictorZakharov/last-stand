...
- **Loading screen:** `#loading` is styled inline in `index.html` so it paints before the bundle. `boot()` is async: call `loadingStep(label, progress)` before each slow block so the browser paints it (JS blocks the main thread, so only compositor animations keep moving in between). The warm-up uses `compileAsync` (parallel compiling where supported) before its real renders.
