...
- **Bright effects:** light adds up, so the final grade (`core/renderer.ts`) compresses it the way the eye does, so a stack of effects never turns white. Glare darkens its surroundings (bloom's blurriest level, `GLARE_ADAPT`), and above `HI_KNEE` the brightest channel eases towards `HI_CAP` with the others scaled with it, so a colour stays its colour. Keep effects and their lights at full strength and let that do the work. Two exceptions:
  - **Large glowing surfaces** (telegraphs, ground flashes, aura rings, big shockwaves) in a colour other than violet, which the effects are tuned on, scale by `glowScale(color)` (`core/materials.ts`): at the same strength a green one is nearly three times as bright.
  - **Small glows** in such a colour (eyes, cores, bolts) take part of it (`glowScale(color, SMALL_GLOW)`), so they still bloom.
  - **The close views:** effects are tuned for the top-down camera 30m away, so bloom is weaker in third and first person (`BLOOM`). Check big effects around the player in first person too: particles fade within 3.5m of the camera, hostile bolt cores hide within 2.5m, and a cast leaves from `Player.castPoint` / `palmPoint`, which in first person is a hand's place in view (the hidden body's hands are at the camera). Keep lights off the camera: above the head, not at 2m.
  - Nothing large may flash or pulse faster than about 1.5 times a second: 3+ flashes a second is a photosensitivity hazard.
