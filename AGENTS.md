...
- **Bright effects:** light adds up, so the final grade (`core/renderer.ts`) compresses it the way the eye does, and effects and their lights stay at full strength: don't dim single effects to fix a bright scene. Glare darkens its surroundings (bloom's blurriest level, `GLARE_ADAPT`); above `HI_KNEE` the brightest channel eases towards `HI_CAP` with the others scaled with it, so stacked light stops adding up; and in a large glare tone mapping keeps a bright pixel's colour (`HUE_*`, `GLARE_FROM/TO`), so a big bright area stays green or blue instead of turning white, while a small hot core may still go white.
