...
- **Loading screen:** `#loading` (markup, SVG logo and styles) is inline in `index.html` so it paints before the bundle. The game renders behind it but only advances once it fades (`revealed` in `main.ts`), so the lobby intro is seen. Tips are in `data/tips.ts`. Its exit (`ui/logoWind.ts`) snapshots the SVG logo (the font is inlined, since an SVG image can't load web fonts), turns every pixel into a WebGL point and blows them away. The motion is closed-form in the vertex shader, and it falls back to a plain fade. `boot()` is async: call `loadingStep(label, progress)` before each slow block so the browser paints it (JS blocks the main thread, so only compositor animations keep moving in between). The warm-up uses `compileAsync` (parallel compiling where supported) before its real renders.
