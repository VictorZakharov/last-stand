...
- **Bright effects:** light adds up, so the final grade (`core/renderer.ts`) compresses it the way the eye does, and effects and their lights stay at full strength: don't dim single effects to fix a bright scene.
  - **Adaptation:** before bloom, the average light over a 16x9 and a 4x3 grid of the frame (`AreaPass`) is held under per-view targets (`ADAPT`, just above each view's ordinary scenes, which it leaves untouched) by scaling only the light above `ADAPT_BASE`. Never scale a whole pixel by its area's brightness: the dim ground round an effect then darkens into a shadow around it. Above `HI_KNEE` the brightest channel eases towards `HI_CAP`, and in a large glare tone mapping keeps a bright pixel's colour (`HUE_*`, `GLARE_*`), so a big bright area stays green or blue instead of turning white.
  - **Distance:** effect materials dim with their distance from the camera (`nearGlow` in `core/materials.ts`: full strength from the top-down camera's distance, down to a quarter up close). `additive()`, the glow particles, projectile cores and the hit flash on bodies have it; wrap a new effect material in `nearGlow()`. The world's own glows don't.
