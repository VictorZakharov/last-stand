...
- **Bright effects:** effect strengths are tuned on violet, and the same strength in green, pale blue or white looks several times brighter and blooms to white. Scale a glow in any other colour by `glowScale(color)` (`core/materials.ts`). The sanitize pass eases pixels above 3 towards 8 before bloom, a safety net for stacked effects. Nothing large may flash or pulse faster than about 1.5 times a second (3+ flashes a second is a photosensitivity hazard). Particles fade within 1.6m of the camera, and check big effects around the player in first person too.
