...
- **Bright effects:** light adds up, so the final grade (`core/renderer.ts`) compresses it the way the eye does, and effects and their lights stay at full strength: don't dim single effects to fix a bright scene.
  ...
  - **Distance:** effect materials dim with their distance from the camera (`nearGlow` in `core/materials.ts`: full strength from the top-down camera's distance, down to a quarter up close). `additive()`, the glow particles, projectile cores (a hostile bolt's is an orb of added light, with a pooled light when one is free), enemies' glows (`kit.glow`; the heroes' opt out) and the hit flash on bodies have it; wrap a new effect material in `nearGlow()`. The world's own glows don't.
